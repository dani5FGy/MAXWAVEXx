...
const GameModule = () => {
  ...
  function update() {
    ...
    const obstacleFrequency = Math.max(2000 - (gameData.level * 100), 800);
  ...
  function createWave(scene) {
    ...
    wave.setVelocityX(-150 - (scene.gameData.level * 10));
    wave.setScale(0.8);
  }

  function createObstacle(scene) {
    const y = Phaser.Math.Between(50, 550);
    const obstacle = scene.gameData.obstacles.create(850, y, 'obstacle');
    obstacle.setVelocityX(-200 - (scene.gameData.level * 15));
  }
